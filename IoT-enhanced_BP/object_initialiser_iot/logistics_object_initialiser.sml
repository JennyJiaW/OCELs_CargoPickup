...
fun oas_by_type(ot: ObjectType) = 
	...
	if ot="Truck" then ["RFID No", "Pickup Plan ID", "Cargo ID", "LPT", "Axles", "Scheduled Pickup Weight", "Truck Status", "Truck Weight", "Is_normal"] else 
	if ot="Cargo" then ["Cargo Type", "Cargo stock weight(scheduled)", "Silo ID"] else 
	if ot="Silo" then ["Silo Status", "Temperature", "Humidity", "Silo Temperature", "Grain Temperature"] else 
	[];
